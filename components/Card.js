...
export function PosterCard({ item }) {
  ...
  return (
    <Link href={href} className="card-poster">
      ...
      {!item.trending && item.daily_pick && <span className="badge badge-pick">⭐ Pick</span>}
...
export function ToolCard({ item }) {
  ...
  return (
    <div className="card" onClick={() => window.location.href = href} style={{ cursor: 'pointer' }}>
      <div className="card-icon">{(item.name || '?').charAt(0)}</div>
      <div className="card-title">{item.name}</div>
      {item.pricing && (
        <span className={isFree ? 'tag-free' : 'tag-paid'} style={{ display: 'inline-block', marginBottom: '10px' }}>
          {item.pricing}
        </span>
      )}
      <p className="card-desc">{item.short_desc || ''}</p>
      <div className="card-actions">
        <Link href={href} className="btn-secondary" style={{ fontSize: '12px', padding: '7px 14px' }} onClick={e => e.stopPropagation()}>Details</Link>
        <a href={item.affiliate_link || href} target="_blank" rel="noopener" className="btn-primary" style={{ fontSize: '12px', padding: '7px 14px' }} onClick={e => e.stopPropagation()}>Get →</a>
      </div>
    </div>
  );
}
...
export default function Card({ item }) {
  if (!item) return null;
  if (['movie', 'book', 'game'].includes(item.type)) return <PosterCard item={item} />;
