// packages/shared/lib/pipeline/GitHubProvider.js
// NovaHub â€” GitHub Trending Provider
...
// Map GitHub topics/languages â†’ NovaHub categories
function mapCategory(repo) {
  ...
  if (
    ...
    topics.includes("hacking")
  )
    return "security";
  if (topics.includes("game") || topics.includes("gamedev")) return "games";
  if (
    lang === "python" &&
    topics.some((t) => ["data", "ml", "deep-learning"].includes(t))
  )
    return "ai-tools";
...
export class GitHubProvider extends BaseProvider {
  ...
  async fetch() {
    ...
    // Get trending repos via search â€” sorted by stars gained recently
    const date = new Date();
