// packages/shared/lib/pipeline/BaseProvider.js
// NovaHub â€” AI Content Pipeline
// Abstract base class for all data providers.
// Subclasses must implement fetch() and transform().

/**
 * BaseProvider â€” the foundation of every content provider in the pipeline.
 *
 * Subclass contract:
 *   - override fetch()      â†’ return raw API response data
 *   - override transform()  â†’ convert raw data into NovaHub item objects
 *
 * The run() template method calls fetch â†’ transform and handles errors.
 */
export class BaseProvider {
  /**
   * @param {string} name â€” human-readable provider name (used in logs)
   */
  constructor(name) {
    if (new.target === BaseProvider) {
      throw new Error(
        "BaseProvider is abstract â€” extend it, don't instantiate it directly.",
      );
    }
    this.name = name;
   ...
   * @param {any} rawData â€” whatever fetch() returned
  ...
  /**
   * Template method â€” runs the full provider lifecycle.
