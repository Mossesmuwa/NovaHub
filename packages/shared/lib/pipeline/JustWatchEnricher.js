...
// Uses JustWatch's public GraphQL API â€” no key needed.
...
export async function enrichWithStreaming(country = "US") {
  ...
  for (const movie of movies) {
    try {
      ...
      if (updateErr) {
        ...
        console.log(
          `${tag} âœ“ ${movie.name} â€” streaming on: ${flatrate.join(", ") || "none"}`,
        );
        stats.enriched++;
      }
  ...
  console.log(
    `${tag} Done â€” enriched: ${stats.enriched}, not found: ${stats.notFound}`,
  );
  return stats;
}
