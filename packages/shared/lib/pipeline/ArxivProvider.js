// packages/shared/lib/pipeline/ArxivProvider.js
// Fetches trending research papers from arXiv â€” no API key needed.
...
export class ArxivProvider extends BaseProvider {
  ...
  async fetch() {
    ...
    for (const subject of SUBJECTS.slice(0, 3)) {
      try {
        ...
        // Parse XML manually â€” no xml2js needed
  ...
  transform(papers) {
    return papers.map((paper) => {
      ...
      if (
        paper.subject.startsWith("cs.AI") ||
        paper.subject.startsWith("cs.LG")
      )
        category = "ai-tools";
      ...
      return {
        ...
        tags: ["arxiv", "research", "paper", paper.subject.toLowerCase()].slice(
          0,
          8,
        ),
        vibe_tags: [],
