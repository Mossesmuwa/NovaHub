...
// Map RAWG genres â†’ NovaHub tags
const GENRE_TAGS = {
  action: "action",
  indie: "indie",
  adventure: "adventure",
...
/**
 * RAWGProvider â€” pulls top-rated recent games from RAWG.io.
 *
 * Environment:
 *   RAWG_API_KEY
 *
...
export class RAWGProvider extends BaseProvider {
   ...
   * @param {number} [options.limit=40]            â€” max games to fetch
   * @param {string} [options.ordering='-rating']  â€” RAWG ordering param
   */
  constructor(options = {}) {
    super("RAWG");
  ...
  async fetch() {
    ...
    // Multiple endpoints â†’ ensures we always get data
    const endpoints = [
