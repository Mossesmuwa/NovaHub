...
export class SpotifyProvider extends BaseProvider {
  ...
  async fetch() {
    ...
    // Featured playlists â€” always trending
    const playlistRes = await fetch(
      `${SPOTIFY_BASE}/browse/featured-playlists?limit=10`,
      { headers },
    );
    ...
    // New releases
    const newRes = await fetch(`${SPOTIFY_BASE}/browse/new-releases?limit=20`, {
      headers,
    });
    ...
    if (all.length === 0) {
      const searchRes = await fetch(
        `${SPOTIFY_BASE}/search?q=music&type=album&limit=20`,
        { headers },
      );
  ...
  transform(items) {
    return items.map((item) => {
      ...
      return {
        ...
        name,
        short_desc: artists
          ? `${item.album_type || "Item"} by ${artists}`
          : item.album_type || "Item",

        long_desc: artists
          ? `${name} by ${artists}. Released ${item.release_date || "recently"}.`
          : name,

        category_id: "music",
        type: item.album_type || "item",
        image,
        ...
        tags: ["spotify", "music", item.album_type || "item"]
          .filter(Boolean)
          .slice(0, 8),
