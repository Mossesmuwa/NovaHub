// packages/shared/lib/pipeline/AIService.js
...
/**
 * AIService â€” Gemini-powered content enrichment.
 *
...
export class AIService {
   ...
   * @param {string} [options.model='gemini-1.5-flash'] â€” Gemini model ID
   * @param {number} [options.maxRetries=1]             â€” retries on failure
   */
  constructor(options = {}) {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      throw new Error(
        "[AIService] GEMINI_API_KEY environment variable is not set.",
      );
    }
   ...
   * @param {string} description â€” raw item description
...
1. "summary": A vibe-heavy, evocative summary in EXACTLY 20 words. 
   Make it feel like a friend recommending something â€” punchy, opinionated, aesthetic.
   
2. "vibe_scores": An object with exactly three keys, each scored 0â€“100:
...
"""`;
   ...
   * @param {string} text â€” raw response text from Gemini
  ...
  _parseResponse(text) {
    ...
    if (typeof parsed.vibe_scores !== "object" || parsed.vibe_scores === null) {
      throw new Error(
        'Missing or invalid "vibe_scores" field in Gemini response.',
      );
    }

    // Clamp scores to 0â€“100
  ...
  /**
   * Clamp a value to the 0â€“100 integer range.
   * @param {any} val
   * @returns {number}
   */
  _clamp(val) {
   ...
   *
   * @param {string} description â€” the raw text to analyze
  ...
  async enrich(description) {
    if (
      ...
      console.warn(
        `${this.tag} Skipping enrichment â€” description too short or missing.`,
      );
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          ...
          console.log(
            `${this.tag} Retry ${attempt}/${this.maxRetries} after ${backoff}ms...`,
          );
        ...
        lastError = err;
        console.error(
          `${this.tag} Attempt ${attempt + 1} failed:`,
          err.message,
        );
      }
    }
    ...
    console.error(
      `${this.tag} All attempts exhausted. Using defaults. Last error:`,
      lastError?.message,
    );
    return this._defaultResult();
  }
