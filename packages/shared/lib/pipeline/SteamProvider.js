...
// Fetches top Steam games â€” no API key required for public data.
...
export class SteamProvider extends BaseProvider {
  ...
  async fetch() {
    // Steam top sellers â€” public endpoint, no key needed
    const endpoints = [
      'https://store.steampowered.com/api/featuredcategories?cc=us&l=en',
    ];

    // Use Steam charts for trending â€” public JSON
    ...
    return rawGames
      ...
      .map(game => {
        ...
        return {
          slug,
          name:         game.name,
          short_desc:   game.genre ? `${game.genre} Â· ${game.positive?.toLocaleString() || 0} positive reviews` : '',
          long_desc:    '',
          category_id:  'games',
          type:         'game',
