...
// No API key needed â€” Wikipedia REST API is free and open.
...
export async function enrichWithWikipedia() {
  ...
  for (const item of items) {
    try {
      ...
      if (updateErr) {
        ...
        console.log(`${tag} âœ“ ${item.name}`);
        stats.enriched++;
      }

      // Wikipedia rate limit â€” be polite
  ...
  console.log(`${tag} Done â€” enriched: ${stats.enriched}, skipped: ${stats.skipped}`);
  return stats;
}
