...
export async function enrichMovies() {
  ...
  for (const movie of movies) {
    try {
      ...
      if (updateErr) {
        console.error(
          `${tag} Update failed for ${movie.name}:`,
          updateErr.message,
        );
        stats.errors.push(updateErr.message);
      } else {
        console.log(
          `${tag} âœ“ ${movie.name} â€” IMDB: ${update.imdb_rating} RT: ${update.rt_score}% MC: ${update.metacritic_score}`,
        );
        stats.enriched++;
      }

      // Rate limit â€” OMDB free tier is 1000/day, be safe
  ...
  console.log(
    `${tag} Done â€” enriched: ${stats.enriched}, not found: ${stats.notFound}, skipped: ${stats.skipped}`,
  );
  return stats;
}
