...
// NovaHub â€” AI Content Pipeline
// Orchestrates: Provider.run() â†’ AIService.enrich() â†’ Supabase upsert.
...
/**
 * SyncEngine â€” the central orchestrator of the content pipeline.
 *
 * Workflow:
...
export class SyncEngine {
   ...
   * @param {number} [options.batchSize=5]      â€” items per AI enrichment batch
   * @param {number} [options.batchDelayMs=1000] â€” delay between batches (ms)
   * @param {boolean} [options.skipAI=false]     â€” skip AI enrichment entirely
   */
  constructor(options = {}) {
    this.tag = "[SyncEngine]";

    // â”€â”€ Supabase admin client (Service Role Key bypasses RLS) â”€â”€
    if (!supabaseAdmin) {
      throw new Error(
        `${this.tag} Admin client not initialized. Ensure SUPABASE_SERVICE_ROLE_KEY is set.`,
      );
    }
  ...
  async _enrichItem(item) {
    ...
    try {
      ...
      console.error(
        `${this.tag} AI enrichment failed for "${item.name}":`,
        err.message,
      );
  ...
  async _enrichBatch(items) {
    ...
    for (let i = 0; i < items.length; i += this.batchSize) {
      ...
      console.log(
        `${this.tag} Enriching batch ${batchNum}/${totalBatches} (${batch.length} items)...`,
      );
  ...
  async _upsert(items) {
    ...
    for (let i = 0; i < items.length; i += UPSERT_BATCH) {
      ...
      try {
        const { error } = await this.supabase.from("items").upsert(batch, {
          onConflict: "slug",
   ...
   * @param {import('./BaseProvider.js').BaseProvider} provider
   * @param {Object} [options]
   * @param {boolean} [options.skipAI] â€” override engine-level skipAI for this run
   * @returns {Promise<Object>} { provider, synced, failed, errors, totalItems }
   */
  async syncProvider(provider, options = {}) {
    const startTime = Date.now();
    console.log(
      `${this.tag} â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•`,
    );
    ...
    try {
      ...
      if (!items.length) {
        console.log(
          `${this.tag} No items returned from ${provider.name}. Skipping.`,
        );
        return {
      ...
      console.log(
        `${this.tag} âœ“ ${provider.name} complete in ${elapsed}s â€” synced: ${stats.synced}, failed: ${stats.failed}`,
      );
      ...
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      console.error(
        `${this.tag} âœ— ${provider.name} failed after ${elapsed}s:`,
        err.message,
      );
      return {
        provider: provider.name,
  ...
  async syncAll() {
    console.log(
      `${this.tag} â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”`,
    );
    console.log(
      `${this.tag} Starting full pipeline sync (${this.providers.length} providers)`,
    );
    console.log(
      `${this.tag} â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”`,
    );
    ...
    console.log(
      `${this.tag} Pipeline complete â€” synced: ${totalSynced}, failed: ${totalFailed}`,
    );
    console.log(
      `${this.tag} â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”`,
    );
