...
// All endpoints to pull from â€” 6 movie + 6 TV
...
export class TMDBProvider extends BaseProvider {
   ...
   * @param {number} [options.pages=3]  â€” pages per endpoint (20 items/page)
   * @param {number} [options.limit=20] â€” legacy single-page limit (ignored when pages > 1)
   */
  constructor(options = {}) {
    super("TMDB");
  ...
  async _fetchPage(path, token, page) {
    try {
      ...
      if (!res.ok) {
        console.warn(`[TMDB] ${path} p${page} â†’ ${res.status}`);
        return [];
      }
  ...
  async fetch() {
    ...
    console.log(
      `[TMDB] Raw fetched â€” movies: ${allMovies.length}, tv: ${allTV.length}`,
    );
