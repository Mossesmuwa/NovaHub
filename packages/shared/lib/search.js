...
//
// Usage:
//   import { searchItems, semanticSearch } from 'shared/lib/search';
//   const results = await searchItems('minimal productivity tool', { category: 'productivity' });

import { supabase } from "./supabase";
import { getEnvCredential } from "./helpers";
...
export async function searchItems(
  query,
  { category = null, type = null, limit = 20, offset = 0 } = {},
) {
  ...
  console.warn(
    "[search] RPC not available, falling back to ILIKE:",
    rpcError?.message,
  );
  ...
  let q = supabase
    .from("items")
    .select(
      "id, slug, name, type, category, short_desc, image, rating, pricing, tags",
    )
    .eq("approved", true)
...
//   2. Set NEXT_PUBLIC_OPENAI_API_KEY or NEXT_PUBLIC_OPENAI_ACCESS_TOKEN in Vercel env vars (for embeddings)
...
export async function semanticSearch(
  query,
  { category = null, threshold = 0.7, limit = 10 } = {},
) {
  if (!query?.trim()) return { data: [], error: null };

  const openaiKey = getEnvCredential(
    "NEXT_PUBLIC_OPENAI_API_KEY",
    "NEXT_PUBLIC_OPENAI_ACCESS_TOKEN",
  );

  if (!openaiKey) {
    console.warn(
      "[search] semanticSearch called but OPENAI_API_KEY not set. Falling back to FTS.",
    );
...
export async function hybridSearch(query, options = {}) {
  ...
  const seen = new Map();
  for (const item of vecItems)
    seen.set(item.id, { ...item, _source: "vector" });
