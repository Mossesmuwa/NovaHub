...
function calculateExternalScore(item) {
  ...
  if (metadata.reddit_upvotes) {
    const redditScore = Math.min(
      100,
      Math.log10(metadata.reddit_upvotes + 1) * 25,
    );
    total += redditScore;
    count++;
    confidence += 15;
...
export async function updateTrendingScores() {
  try {
    ...
    console.log(
      `Updated trending scores for ${trendingItems?.length || 0} items`,
    );
...
const WEIGHTS = {
  ...
  loved: 0.2, // item name/slug appears in user's loved list
  mood: 0.1, // item vibe_tags match user's current mood goal
  quality: 0.1, // item's own rating (normalised to 0–1)
};
...
function lovedScore(item, taste) {
  ...
  return loved.some(
    (l) => name.includes(l) || slug.includes(l) || l.includes(name),
  )
    ? 1.0
    : 0.0;
}
