  ...
  return (
    <Link
      ...
      style={{
        ...
        background: hovered
          ? "var(--bg3)"
          : `linear-gradient(135deg, ${MEDAL_GLOW[rank]} 0%, var(--bg2) 40%)`,
      }}
    ...
    >
      ...
      >
        ...
        >
          🔥{" "}
          {item.trending_score
            ? `${item.trending_score.toFixed(0)} pts`
            : "Hot"}
        </span>
      </div>
    </Link>
...
export default function TrendingPage() {
  ...
  const filtered =
    cat === "all"
      ? items
      : items.filter((i) => (i.category_id || i.category) === cat);
  ...
  return (
    <Layout activePage="trending">
      <SEO
        title="Trending — NovaHub"
        description="What's trending on NovaHub right now."
      />
      ...
      <div
        style={{
          textAlign: "center",
          padding: "calc(var(--nav)+48px) 20px 36px",
          position: "relative",
          overflow: "hidden",
        }}
      >
        <div
          style={{
            ...
            inset: 0,
            background:
              "radial-gradient(ellipse 60% 50% at 50% 0%,rgba(201,168,76,.09) 0%,transparent 60%)",
            pointerEvents: "none",
          }}
        />
        <div style={{ position: "relative", zIndex: 1 }}>
          ...
          >
            Ranked by saves, clicks, and recency — updated every night by Nova
            Pulse.
          </p>
        </div>
      </div>
      ...
      <div className="container" style={{ paddingBottom: 80 }}>
        ...
        {loading ? (
          <div
            style={{ textAlign: "center", padding: "80px", color: "var(--t3)" }}
          >
            <div
              style={{
                width: 28,
          ...
          <>
            ...
            {top3.length > 0 && (
              ...
              >
                {top3.map((item, i) => (
                  <PodiumCard key={item.id} item={item} rank={i} />
                ))}
              </div>
            )}
            ...
            {rest.length > 0 && (
              <>
                ...
                >
                  {rest.map((item, i) => (
                    <GridCard key={item.id} item={item} rank={i + 4} />
                  ))}
                </div>
              </>
            )}
