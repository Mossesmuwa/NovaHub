...
async function fetchTrending(limit) {
  ...
  if (data.errors) throw new Error(data.errors[0]?.message || 'PH GraphQL error');
...
function phToItem(post) {
  ...
  const tags     = ['product-hunt', 'tool', ...topics.map(t => slugify(t.name))].slice(0, 8);
  ...
  return {
    ...
    year:         post.createdAt ? new Date(post.createdAt).getFullYear() : new Date().getFullYear(),
    rating:       null,
