...
export default async function handler(req, res) {
  ...
  if (!process.env.PRODUCTHUNT_DEVELOPER_TOKEN) {
    return res.status(500).json({ error: 'PRODUCTHUNT_DEVELOPER_TOKEN not set' });
  }

  try {
    const posts = await fetchPHTrending(BATCH_SIZE);
    if (!posts.length) {
      return res.status(200).json({ success: true, count: 0, message: 'No posts returned' });
    }
    ...
    return res.status(200).json({ success: true, count, sample: items[0]?.name });
