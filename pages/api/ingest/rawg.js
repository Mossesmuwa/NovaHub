...
async function rawgFetch(path, params = {}) {
  const qs = new URLSearchParams({ key: process.env.RAWG_API_KEY, ...params }).toString();
...
function gameToItem(g) {
  ...
  return {
    ...
    platforms,
    tags:        ['rawg', 'game', ...(g.genres || []).map(gen => gen.slug)].slice(0, 8),
    vibe_tags:   [],
...
export default async function handler(req, res) {
  ...
  try {
    // Top-rated all time
    const top  = await rawgFetch('/games', { ordering: '-rating', page_size: BATCH_SIZE, metacritic: '80,100' });
    const items = (top.results || []).map(gameToItem);
    if (items.length) results.topRated = await upsertItems(items);
  } catch (err) { results.errors.push(`TopRated: ${err.message}`); }

  try {
    // Recently released
    const recent = await rawgFetch('/games', { ordering: '-released', dates: `${past90},${today}`, page_size: BATCH_SIZE });
    const items  = (recent.results || []).map(gameToItem);
    if (items.length) results.recent = await upsertItems(items);
  } catch (err) { results.errors.push(`Recent: ${err.message}`); }
