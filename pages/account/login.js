...
export default function LoginPage() {
  ...
  async function doLogin(e) {
    ...
    setLoading(false);
    if (!result.success) { setErr(result.error === 'Invalid login credentials' ? 'Incorrect email or password.' : result.error); return; }
    setOk('✦ Signed in! Redirecting…');
    const ret = router.query.return || '/account/dashboard';
    setTimeout(() => router.push(ret), 800);
  }
