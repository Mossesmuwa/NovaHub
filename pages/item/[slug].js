...
export default function ItemPage() {
  ...
  async function getAlternatives(item) {
    try {
      // For now, get AI-powered alternatives. Later this can use the alternatives table
      const res = await fetch("/api/ai-recommend", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode: "alternatives", item: item, limit: 4 }),
      });
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setItem(data);
      setLoading(false);
      const [rel, alts, saved, cmts] = await Promise.all([
  ...
  let ctaBtn = null;
  if (item.type === "movie" && item.watch_link)
    ctaBtn = (
      <a
  ...
  else if (item.type === "book" && item.buy_link)
    ctaBtn = (
      <a
        href={item.buy_link}
        target="_blank"
  ...
  else if (item.type === "game" && item.play_link)
    ctaBtn = (
      <a
        href={item.play_link}
        target="_blank"
  ...
  return (
    <Layout>
      <SEO
        ...
        description={item.short_desc || `${item.name} on NovaHub`}
      />
      <div className="progress-bar" id="progress-bar"></div>
      ...
      <div className="item-hero">
        ...
        <div className="container">
          <div className="breadcrumb">
            ...
            <Link href={`/category?cat=${item.category_id}`}>
              {cat.icon} {cat.name}
            </Link>
            <span className="breadcrumb-sep">›</span>
            <span>{item.name}</span>
          ...
          <div className="section" style={{ paddingTop: 0 }}>
            <div className="section-header">
              <div>
                ...
                <h2 className="section-title">
                  Better Options & Similar Tools
                </h2>
              </div>
              <Link
                href={`/search?q=${encodeURIComponent(item.name)}+alternative`}
            ...
            <div className="grid-4 stagger">
              {alternatives.map((alt) => (
                <div className="reveal-scale" key={alt.id || alt.name}>
                  <Card item={alt} />
                  {alt.similarity_score && (
                    <div
                      style={{
                        ...
                        textAlign: "center",
                        marginTop: "4px",
                        opacity: alt.similarity_score > 70 ? 1 : 0.7,
                      }}
                    >
                      {alt.similarity_score > 70
                        ? "Very similar"
                        : alt.similarity_score > 40
                          ? "Similar approach"
      ...
      {related.length > 0 && (
        <div className="container">
          <div className="section" style={{ paddingTop: 0 }}>
            ...
            <div className="grid-4 stagger">
              {related.map((r) => (
                <div className="reveal-scale" key={r.id}>
                  <Card item={r} />
                </div>
              ))}
            </div>
