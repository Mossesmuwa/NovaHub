...
const PRIORITY_SERVICES = [
  'netflix', 'disney', 'hbo', 'hulu', 'amazon', 'apple',
  'paramount', 'peacock', 'showtime', 'mubi'
];
...
export async function enrichWithStreaming(country = 'US') {
  ...
  for (const movie of movies) {
    try {
      ...
      const flatrate = offers.filter(o => o.type === 'FLATRATE').map(o => o.service);
      const rent     = offers.filter(o => o.type === 'RENT').map(o => o.service);
      const free     = offers.filter(o => o.type === 'FREE').map(o => o.service);
      ...
      const updatedMetadata = {
        ...(existing?.metadata || {}),
        streaming: { flatrate, rent, free, updated_at: new Date().toISOString() },
      };
      ...
      if (updateErr) {
        ...
        console.log(`${tag} ✓ ${movie.name} — streaming on: ${flatrate.join(', ') || 'none'}`);
        stats.enriched++;
      }
  ...
  console.log(`${tag} Done — enriched: ${stats.enriched}, not found: ${stats.notFound}`);
  return stats;
}
