...
export async function loginWithGoogle(redirectTo) {
  ...
  const { error } = await supabase.auth.signInWithOAuth({
    provider: 'google', options: { redirectTo: redirectTo || window.location.origin + '/account/dashboard' },
  });
  if (error) return { success: false, error: error.message };
  return { success: true };
}
...
export async function loginWithGithub(redirectTo) {
  if (!supabase) return { success: false, error: 'Database not ready.' };
  const { error } = await supabase.auth.signInWithOAuth({
    provider: 'github', options: { redirectTo: redirectTo || window.location.origin + '/account/dashboard' },
  });
  if (error) return { success: false, error: error.message };
  return { success: true };
}
...
export async function getOrCreateAnonSession() {
  ...
  if (existingId) {
    try {
      const { data } = await supabase.from('anon_sessions').select('*')
        .eq('id', existingId).gt('expires_at', new Date().toISOString()).single();
      if (data) {
        supabase.from('anon_sessions').update({ last_seen: new Date().toISOString() })
          .eq('id', existingId).then(() => {}).catch(() => {});
        return data;
      }
    } catch {}
  }
  try {
    const { data, error } = await supabase.from('anon_sessions').insert({
      fingerprint: _fingerprint(),
      expires_at: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
    }).select().single();
    if (!error && data) { _setAnonId(data.id); return data; }
  } catch {}
  return null;
}
...
export async function checkAnonLimit(type) {
  ...
  try {
    const { data } = await supabase.from('anon_sessions')
      .select('favorites_count, comments_today, last_comment_date').eq('id', id).single();
    if (!data) return { allowed: true };
    if (type === 'favorites' && data.favorites_count >= ANON_LIMITS.maxFavorites) {
      return { allowed: false, showUpgrade: true, reason: `You've reached the ${ANON_LIMITS.maxFavorites} item limit for anonymous users. Create a free account to save unlimited items.` };
    }
    if (type === 'comments') {
      ...
      if (todayCount >= ANON_LIMITS.maxCommentsPerDay) {
        return { allowed: false, showUpgrade: true, reason: `You've reached ${ANON_LIMITS.maxCommentsPerDay} comments today. Sign up for unlimited comments.` };
      }
    }
  } catch {}
  return { allowed: true };
...
export async function mergeAnonData(userId) {
  ...
  if (!id) return;
  try {
    const { data: favs } = await supabase.from('favorites').select('item_id').eq('anon_id', id);
    if (favs && favs.length) {
