...
//   1. AI Enrichment (Gemini 1.5 Flash) — summary + vibe_scores
...
const ENRICHMENT_MODEL = 'gemini-1.5-flash';
...
export class Pipeline {
  ...
  constructor(providerName, defaultCategory = 'ai-tools') {
    if (new.target === Pipeline) {
      throw new Error('Pipeline is abstract — extend it, don\'t instantiate it directly.');
    }
    ...
    if (!geminiKey) {
      console.warn(`${this.tag} GEMINI_API_KEY not set — AI enrichment and embeddings will be skipped.`);
      this.ai = null;
    } else {
    ...
    // ── Stats tracking ──
    this._stats = { fetched: 0, enriched: 0, embedded: 0, inserted: 0, updated: 0, failed: 0, errors: [] };
  }
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // AI ENRICHMENT — Gemini 1.5 Flash
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  async _enrichWithAI(description) {
    if (!this.ai || !description || description.trim().length < 10) {
...
"""`;
  ...
  async _generateEmbedding(text) {
    if (!this.ai || !text || text.trim().length < 5) {
      return null;
    }
  ...
  async _processAIBatches(items) {
    if (!this.ai) {
      console.log(`${this.tag} Skipping AI processing — no GEMINI_API_KEY.`);
      return items;
    }
    ...
    for (let i = 0; i < items.length; i += AI_BATCH_SIZE) {
      ...
      console.log(`${this.tag} AI batch ${batchNum}/${totalBatches} (${batch.length} items)...`);
      ...
      const results = await Promise.all(
        batch.map(async (item) => {
          try {
            ...
            const embeddingText = `${item.name || ''} ${item.short_desc || ''} ${description}`.trim();
            const embedding = await this._generateEmbedding(embeddingText);

            const enriched = {
              ...item,
              short_desc: enrichment.summary || item.short_desc,
              vibe_scores: Object.keys(enrichment.vibe_scores).length > 0
                ? enrichment.vibe_scores
                : undefined,
              vibe_tags: Object.entries(enrichment.vibe_scores)
                .map(([k, v]) => `${k}:${v}`),
            };
            ...
            console.error(`${this.tag} AI processing failed for "${item.name}":`, err.message);
            return item;
          }
        })
      );
  ...
  async _batchUpsert(items) {
    ...
    for (let i = 0; i < items.length; i += UPSERT_BATCH_SIZE) {
      ...
      console.log(`${this.tag} DB upsert batch ${batchNum}/${totalBatches} (${batch.length} items)...`);
      ...
      try {
        ...
        const newCount = batch.filter((item) => !existingSlugs.has(item.slug)).length;
        ...
        console.error(`${this.tag} Upsert batch ${batchNum} failed:`, err.message);
        failed += batch.length;
  ...
  async sync() {
    ...
    try {
      ...
      // ── Step 5: Load ──
      console.log(`${this.tag} [5/5] Upserting to Supabase (batch size: ${UPSERT_BATCH_SIZE})...`);
      ...
      console.log(`${this.tag} ═══════════════════════════════════════════════`);
      console.log(`${this.tag} ✓ Sync complete`, JSON.stringify(report, null, 2));
      console.log(`${this.tag} ═══════════════════════════════════════════════`);
