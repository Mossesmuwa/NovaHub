...
"""`;
  ...
  async _processAIBatches(items) {
    ...
    // Validate all items have required fields BEFORE AI processing
    console.log(`${this.tag} Validating ${items.length} items...`);
    const invalidItems = [];
    for (let i = 0; i < items.length; i++) {
      ...
      if (!item.category_id) errors.push("missing category_id");
      if (errors.length > 0) {
        invalidItems.push({ index: i, name: item.name || "(unnamed)", errors });
      }
    }
    if (invalidItems.length > 0) {
      console.warn(`${this.tag} ⚠ Found ${invalidItems.length} invalid items:`);
      invalidItems.slice(0, 5).forEach((inv) => {
        console.warn(
          `${this.tag}   [${inv.index}] ${inv.name}: ${inv.errors.join(", ")}`,
        );
      });
    }
  ...
  async _batchUpsert(items) {
    ...
    console.log(`${this.tag} _batchUpsert() called with ${items.length} items`);

    // Verify supabaseAdmin is initialized
    if (!supabaseAdmin) {
      console.error(`${this.tag} ✗ CRITICAL: supabaseAdmin is null!`);
      throw new Error(
        "supabaseAdmin is not initialized. Check SUPABASE_SERVICE_ROLE_KEY.",
      );
    }
    console.log(`${this.tag} ✓ supabaseAdmin initialized`);

    // Sanity check: verify first item has required fields
    if (items.length > 0) {
      const first = items[0];
      console.log(`${this.tag} First item sample:`, {
        slug: first.slug,
        name: first.name,
      ...
      if (!first.slug) {
        console.error(`${this.tag} ✗ WARNING: first item has NO SLUG!`);
      }
      if (!first.name) {
        console.error(`${this.tag} ✗ WARNING: first item has NO NAME!`);
      }
      if (!first.category_id) {
        console.error(`${this.tag} ✗ WARNING: first item has NO CATEGORY_ID!`);
      }
    }
    ...
    for (let i = 0; i < items.length; i += UPSERT_BATCH_SIZE) {
      ...
      try {
        ...
        console.log(
          `${this.tag}   Checking existence for ${slugs.length} slugs...`,
        );
        ...
        if (checkError) {
          console.error(
            `${this.tag}   ✗ Error checking existing slugs:`,
            checkError.message,
          );
          throw checkError;
        }
        ...
        console.log(`${this.tag}   Found ${existingSlugs.size} existing slugs`);
        console.log(
          `${this.tag}   New items: ${newCount}, Updates: ${updateCount}`,
        );
        ...
        console.log(`${this.tag}   Upserting ${cleanBatch.length} items...`);
        ...
        if (error) {
          console.error(`${this.tag}   ✗ Upsert error:`, error);
          console.error(`${this.tag}   Error details:`, {
            code: error.code,
        ...
        console.log(
          `${this.tag}   ✓ Upsert successful - inserted ${newCount}, updated ${updateCount}`,
        );
        inserted += newCount;
        updated += updateCount;
      } catch (err) {
        ...
        console.error(`${this.tag}   Full error:`, err);
        failed += batch.length;
    ...
    console.log(
      `${this.tag} _batchUpsert() complete - inserted: ${inserted}, updated: ${updated}, failed: ${failed}`,
    );
  ...
  async sync() {
    ...
    console.log(
      `${this.tag} AI Clients: Claude=${this.claude ? "✓" : "✗"}, Gemini=${this.gemini ? "✓" : "✗"}`,
    );
    console.log(`${this.tag} ═══════════════════════════════════════════════`);
    ...
    try {
      ...
      console.log(`${this.tag} [1/5] ✓ Fetch complete`);
      ...
      console.log(`${this.tag} [2/5] ✓ Transformed ${items.length} items`);

      // ── Step 3 & 4: AI Enrich + Embed ──
      console.log(`${this.tag} [3/5] AI enrichment (Claude)...`);
      console.log(`${this.tag} [4/5] AI embeddings (Gemini)...`);
      const enrichedItems = await this._processAIBatches(items);
      console.log(
        `${this.tag} [3-4/5] ✓ AI processing complete (enriched: ${this._stats.enriched}, embedded: ${this._stats.embedded})`,
      );
