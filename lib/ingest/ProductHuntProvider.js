...
export class ProductHuntProvider extends Pipeline {
  ...
  async fetch() {
    ...
    if (!token) {
      console.error(
        `${this.tag} ✗ CRITICAL: Product Hunt credential not found`,
      );
      throw new Error(
        "Product Hunt credential not set (PRODUCTHUNT_DEVELOPER_TOKEN or PRODUCTHUNT_ACCESS_TOKEN).",
      );
    }

    console.log(
      `${this.tag} Making API call to ${PH_API} with limit=${this.limit}...`,
    );
